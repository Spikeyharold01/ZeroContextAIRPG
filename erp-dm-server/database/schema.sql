-- ============================================================
-- ADAPTIVE RPG/ERP ENGINE – DATABASE SCHEMA VERSION 9
-- ============================================================
...
-- 19. ACCEPTED CONVERSATION EXCHANGES
-- campaigns.current_turn is the accepted conversation exchange sequence number.
CREATE TABLE conversation_turn_requests (
    ...
    active_player_subject_id TEXT NOT NULL,
    response_json TEXT,
    error_code TEXT,
...
CREATE INDEX idx_conversation_turn_requests_status
    ON conversation_turn_requests(campaign_id,status,created_at,id);
CREATE INDEX idx_conversation_turn_requests_lease
    ON conversation_turn_requests(campaign_id,status,lease_expires_at,id);
...
CREATE TABLE conversation_turn_commits (
    ...
    conversation_turn_after INTEGER NOT NULL,
    active_player_subject_id TEXT NOT NULL,
    storyteller_output_hash TEXT NOT NULL,
    visible_narrative_hash TEXT NOT NULL,
    repair_status TEXT NOT NULL CHECK(repair_status IN ('valid','repaired')),
    created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY(campaign_id) REFERENCES campaigns(id),
    FOREIGN KEY(campaign_id,conversation_turn_request_id)
        REFERENCES conversation_turn_requests(campaign_id,id),
    CHECK(conversation_turn_after = conversation_turn_before + 1),
    UNIQUE(campaign_id,conversation_turn_after),
    UNIQUE(campaign_id,conversation_turn_request_id),
    UNIQUE(campaign_id,id)
);
CREATE INDEX idx_conversation_turn_commits_campaign
    ON conversation_turn_commits(campaign_id,conversation_turn_after);
...
CREATE TABLE conversation_turn_messages (
    ...
    created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY(campaign_id) REFERENCES campaigns(id),
    FOREIGN KEY(campaign_id,conversation_turn_commit_id)
        REFERENCES conversation_turn_commits(campaign_id,id),
    UNIQUE(conversation_turn_commit_id,message_index)
);
CREATE INDEX idx_conversation_turn_messages_history
    ON conversation_turn_messages(campaign_id,created_at,conversation_turn_commit_id,message_index);
